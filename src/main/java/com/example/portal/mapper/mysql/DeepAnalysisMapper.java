...
public interface DeepAnalysisMapper {
     ...
     * 插入新的深度分析记录
     */
    @Insert("INSERT INTO deep_analysis (id, analysis_name, jump_url, view_count, status, publish_date, create_date, update_date, create_user, update_user ,category_name ) " +
     ...
     * 更新深度分析记录
     */
    @Update("UPDATE deep_analysis SET analysis_name = #{analysisName}, jump_url = #{jumpUrl}, view_count = #{viewCount}, status = #{status}, " +
    ...
    @Select("SELECT   id, analysis_name, jump_url, view_count, status, publish_date, " +
            "     create_date, update_date, create_user, update_user ,category_name  FROM deep_analysis WHERE analysis_name = #{analysisName}")
    @ResultMap("DeepAnalysisResultMap")
